...
class OPFGraph:
    def __init__(self, nodes, node_limits,edges, G, B, edge_index, Base_MVA):
        ...
        # Nodes
        self.nodes = pd.DataFrame(nodes) #Dictionnary of nodes with information: [bus type, P_d, Q_d, c_1, c_2, c_3] -> Original P_d and Q_d
        self.node_limits = pd.DataFrame(node_limits).T #Dictionnary of nodes with: [P_max, P_min, Q_max, Q_min, V_max, V_min]
        ...
        for n in range(len(edge_index[0])):
            ...
            if i != j:
                if ([i,j] not in actual_edges):
                    ...
                    actual_edge_limits.append([edges[i][j]['Sij_max'],edges[i][j]['line_shunts'], edges[i][j]['theta_max'], edges[i][j]['theta_min']])
    ...
    def GeneratePoints(self, bounds, samples):
        ...
        for i in range(self.node_nb): #This is where we can change the distribution of inputs
            P_distrib = np.random.uniform((1 - bounds)*self.nodes[i]['P_d'],(1 + bounds)*self.nodes[i]['P_d'], size = samples)
            P_distributions.append(P_distrib)
            Q_distrib = np.random.uniform((1 - bounds)*self.nodes[i]['Q_d'],(1 + bounds)*self.nodes[i]['Q_d'], size = samples)
    ...
    def Normalize(self): #Haven't normalized everything yet TODO: Change the max norm to something more centered
        ...
        #"""
        V_norm = np.max(pd.concat([self.node_limits['V_min'],(self.node_limits['V_max'])])) #Structure here is not efficient. Is kept this way in case we want to change norms
        ...
        self.norm_coeffs = {'V_norm': 11, 'P_norm': P_norm, 'Q_norm': Q_norm,'S_norm': S_norm, 'Theta_norm': 360., 'Base_MVA': self.Base_MVA}
        #"""
        print(P_norm)
        print(Q_norm)
    ...
    def GetCharacteristics(self):
        return {'node_nb': self.node_nb, 'total_node_nb': self.node_nb,'nodes': self.nodes, 'node_limits': self.node_limits, 'edge_limits': self.actual_edge_limits, 'edge_index':self.edge_index, 'actual_edges': self.actual_edges, 'G': self.G, 'B': self.B, 'norm_coeffs': self.norm_coeffs,'Ref_node': self.ref_node , 'gen_index': self.gen_index}#,'gen_groups': self.gen_groups, 'original_nodes': self.original_nodes}
    ...
    def DataSplit(self,split, batch_size):
        ...
        for i in range(len(X['X'])):
            ...
            data = Data(x=N, edge_index=self.edge_index, edge_attr= None, y=Y_o).to(device)  #Since all of the rest of the information is shared between all points, no reason to include it in each data point
...
def build_OPFGraph(case,points, spread, split, batch_size):
    ...
    for i in range(nb_nodes):
        ...
        #print('Type:' + str(type))
        if type == 0: #Empty
            nodes[i] = {'type': type, 'P_d': 0.,'Q_d': 0.,'cp_1': 0.,'cp_2': 0.,'cp_3': 0.,'cq_1': 0.,'cq_2': 0.,'cq_3': 0.}
            node_limits[i] = {'P_max': 0.,'P_min': 0.,'Q_max': 0.,'Q_min': 0.,'V_max': mpc['bus'][i][11], 'V_min': mpc['bus'][i][12]}
            #node_limits[i] = {'P_max': 1e-6,'P_min': -1e-6,'Q_max': 1e-6,'Q_min': -1e-6,'V_max': mpc['bus'][i][11], 'V_min': mpc['bus'][i][12]}
        elif type == 1: #Loads
            nodes[i] = {'type': type, 'P_d': mpc['bus'][i][2],'Q_d': mpc['bus'][i][3],'cp_1': 0.,'cp_2': 0.,'cp_3': 0.,'cq_1': 0.,'cq_2': 0.,'cq_3': 0.}
            node_limits[i] = {'P_max': 0.,'P_min': 0.,'Q_max': 0.,'Q_min': 0.,'V_max': mpc['bus'][i][11], 'V_min': mpc['bus'][i][12]}
            #node_limits[i] = {'P_max': 1e-6,'P_min': -1e-6,'Q_max': 1e-6,'Q_min': -1e-6,'V_max': mpc['bus'][i][11], 'V_min': mpc['bus'][i][12]}
        elif type == 2: #Generator
            if Q_case:
                nodes[i] = {'type': type, 'P_d': 0.,'Q_d': 0.,'cp_1': mpc['gencost'][gen_count][4],'cp_2': mpc['gencost'][gen_count][5],'cp_3': mpc['gencost'][gen_count][6],'cq_1': mpc['gencost'][gen_count + nb_gens][4],'cq_2': mpc['gencost'][gen_count + nb_gens][5],'cq_3': mpc['gencost'][gen_count + nb_gens][6] }
            else:
                nodes[i] = {'type': type, 'P_d': 0.,'Q_d': 0.,'cp_1': mpc['gencost'][gen_count][4],'cp_2': mpc['gencost'][gen_count][5],'cp_3': mpc['gencost'][gen_count][6],'cq_1': 0.,'cq_2': 0.,'cq_3': 0.}
            node_limits[i] = {'P_max': mpc['gen'][gen_count][8],'P_min': mpc['gen'][gen_count][9],'Q_max': mpc['gen'][gen_count][3],'Q_min': mpc['gen'][gen_count][4],'V_max': mpc['bus'][i][11], 'V_min': mpc['bus'][i][12]}
            gen_count += 1
        else: # Generator + Load
            if Q_case:
                nodes[i] = {'type': type, 'P_d': mpc['bus'][i][2],'Q_d': mpc['bus'][i][2],'cp_1': mpc['gencost'][gen_count][4],'cp_2': mpc['gencost'][gen_count][5],'cp_3': mpc['gencost'][gen_count][6],'cq_1': mpc['gencost'][gen_count + nb_gens][4],'cq_2': mpc['gencost'][gen_count + nb_gens][5],'cq_3': mpc['gencost'][gen_count + nb_gens][6] }
            else:
                nodes[i] = {'type': type, 'P_d': mpc['bus'][i][2],'Q_d': mpc['bus'][i][2],'cp_1': mpc['gencost'][gen_count][4],'cp_2': mpc['gencost'][gen_count][5],'cp_3': mpc['gencost'][gen_count][6],'cq_1': 0.,'cq_2': 0.,'cq_3': 0.}
            node_limits[i] = {'P_max': mpc['gen'][gen_count][8],'P_min': mpc['gen'][gen_count][9],'Q_max': mpc['gen'][gen_count][3],'Q_min': mpc['gen'][gen_count][4],'V_max': mpc['bus'][i][11], 'V_min': mpc['bus'][i][12]}
            gen_count +=1
    #print(node_limits)
    edges = {}
    ...
    for line in mpc['branch']:
        ...
        edges[int(line[0])-1][int(line[1])-1] = {'Sij_max': line[5],'line_shunts': line[4] ,'theta_max': line[11], 'theta_min': line[12]} #This isn't correct in the original code. It isn't symmetric
        edges[int(line[1])-1][int(line[0])-1] = {'Sij_max': line[5],'line_shunts': line[4] , 'theta_max': line[11], 'theta_min': line[12]}
