...
for j in range(points):
    ...
    attempt_count = 0  # Count: Cumulative attempt count (not successful perturbation count)
    perturbation_count = 0  # Track actual number of successful perturbations
    perturbed_lines = []
    available_lines = list(range(len(mpc['branch'])))

    # While loop continues until max attempts K is reached or no lines available
    while attempt_count < K_max and len(available_lines) > 0:
        # Accumulate attempt count regardless of whether perturbation occurs
        attempt_count += 1
